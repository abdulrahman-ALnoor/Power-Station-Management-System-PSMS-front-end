...
export function ReadingStatusBadge({ status }: ReadingStatusBadgeProps) {
  ...
  switch (status) {
    ...
    case 'pending':
      return (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-warning/10 text-warning dark:bg-amber-500/10 dark:text-amber-500">
          <Clock size={12} />
    ...
    default:
      return (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-outline/10 text-outline">
