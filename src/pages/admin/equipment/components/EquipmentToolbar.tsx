...
export function EquipmentToolbar({ onAddClick }: EquipmentToolbarProps) {
  ...
  return (
    <div className="bg-surface-white dark:bg-surface-container-low p-6 rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] flex flex-wrap items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 justify-end">
        <button 
          onClick={onAddClick}
          className="flex items-center gap-2 px-6 py-2 bg-primary text-on-primary rounded-lg font-bold hover:bg-primary-container dark:bg-primary-fixed dark:text-primary dark:hover:bg-primary dark:hover:text-white transition-colors shadow-md active:scale-95"
        >
          <Plus size={20} />
          <span>{t('toolbar.addEquipment')}</span>
        </button>
        
        <button 
          className="p-2 border border-outline-variant dark:border-border-muted text-on-surface-variant dark:text-outline rounded-lg hover:bg-surface-container dark:hover:bg-surface transition-colors"
