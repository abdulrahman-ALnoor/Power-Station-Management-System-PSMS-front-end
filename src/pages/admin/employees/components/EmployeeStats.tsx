...
export function EmployeeStats({
  ...
  const [data, setData] =
    useState<EmployeeStatsResponse | null>(null)
  ...
  const getVariantStyles = (
    ...
    switch (variant) {
      ...
      case 'steel-blue':
        return 'border-steel-blue text-steel-blue bg-steel-blue/10'

      case 'bright-gold':
        return 'border-bright-gold text-amber-500 bg-bright-gold/10'

      case 'secondary-container':
        return 'border-secondary-container text-yellow-700 bg-secondary-container/10'

      case 'primary-container':
        return 'border-primary-container text-primary-container bg-primary-container/10'
  ...
  if (error) {
    ...
    return (
      <div className="p-4 rounded-xl bg-error/10 text-error text-label-sm">
        {error}
      </div>
    )
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
      ...
      {stats.map((stat) => {
        ...
        const borderColor =
          variantStyle.split(' ')[0]

        const iconColor =
          variantStyle.split(' ')[1]

        const iconBg =
          variantStyle.split(' ')[2]
